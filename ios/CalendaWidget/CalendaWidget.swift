...
extension Color {
    ...
    static func pastelCellBackground(from hex: String?) -> Color {
        let raw = (hex ?? "#A8D5BA").replacingOccurrences(of: "#", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
        var int: UInt64 = 0
        guard Scanner(string: raw).scanHexInt64(&int) else {
            return Color(.sRGB, red: 0.94, green: 0.97, blue: 0.94, opacity: 0.96)
        }
        let r, g, b: Double
        switch raw.count {
        case 3:
        ...
        case 8:
            ...
            b = Double(int & 0xFF)
        default:
            r = 168; g = 213; b = 186 // Soft Sage (#A8D5BA)
        }
...
struct WeeklyWidgetEntryView: View {
    ...
    var selectedColumnFill: Color {
        colorScheme == .dark ? Color.white.opacity(0.18) : Color(hex: "#102E19").opacity(0.08)
    }
